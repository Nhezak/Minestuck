...
public class PunchDesignixTileEntity extends TileEntity
{
	...
	public void checkStates()
	{
		...
		if (!world.getBlockState(getPos().offset(hOffset)).equals(MSBlocks.PUNCH_DESIGNIX.KEYBOARD.get().getDefaultState().with(FACING, facing)) ||
				!world.getBlockState(getPos().down()).equals(MSBlocks.PUNCH_DESIGNIX.LEFT_LEG.get().getDefaultState().with(FACING, facing)) ||
				!world.getBlockState(getPos().down().offset(hOffset)).equals(MSBlocks.PUNCH_DESIGNIX.RIGHT_LEG.get().getDefaultState().with(FACING, facing)))
		{
			broken = true;
		}
	}
