...
public class MinestuckAdvancementProvider implements IDataProvider
{
	...
	MinestuckAdvancementProvider(DataGenerator generator)
	{
		this.generator = generator;
	}
	
	@SuppressWarnings({"ConstantConditions", "unused"})
	private void buildAdvancements(Consumer<Advancement> advancementSaver)
	{
		...
		Advancement goldSeeds = Advancement.Builder.builder().withParent(alchemy).withDisplay(MSBlocks.GOLD_SEEDS.get(), new TranslationTextComponent("advancements.minestuck.gold_seeds.title"), new TranslationTextComponent("advancements.minestuck.gold_seeds.description"), null, FrameType.TASK, true, true, false).withCriterion("plant_gold_seeds", PlacedBlockTrigger.Instance.placedBlock(MSBlocks.GOLD_SEEDS.get())).register(advancementSaver, Minestuck.MOD_ID+":minestuck/gold_seeds");
	...
	private static Advancement.Builder changeModusCriteria(Advancement.Builder builder)
	{
		...
		{
			builder = builder.withCriterion(Objects.requireNonNull(type.getRegistryName()).getPath(), InventoryChangeTrigger.Instance.forItems(type.getStack().getItem()));
		}
		return builder;
	}
	...
	private static Advancement.Builder consumeBugCriteria(Advancement.Builder builder)
	{
		...
		{
			builder = builder.withCriterion(Objects.requireNonNull(item.asItem().getRegistryName()).getPath(), ConsumeItemTrigger.Instance.forItem(item));
		}
		return builder;
	}
